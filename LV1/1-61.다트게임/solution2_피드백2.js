...
// 다트 게임은 다트판에 다트를 세 차례 던져 그 점수의 합계로 실력을
// 겨루는 게임으로, 모두가 간단히 즐길 수 있다.
// 갓 입사한 무지는 코딩 실력을 인정받아 게임의 핵심 부분인 점수 계산
// 로직을 맡게 되었다. 다트 게임의 점수 계산 로직은 아래와 같다.
...
const split = (s) => {
    ...
    for (let i = 0; i < s.length; i++) {
        if (s[i] === 'S' || s[i] === 'D' || s[i] === 'T') {
            if (s[i + 1] === '*' || s[i + 1] === '#') {
                result.push({
                    score: Number(s.substring(lastIndex, i)),
                    zone: s[i],
                    option: s[i + 1]
                });
                lastIndex = i + 2;
...
// console.log(split('1S2D*3T'));

function calc(arr) {
    const zones = {'S': 1, 'D': 2, 'T': 3};
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        score = Math.pow(score, zones[zone]);
        score *= options[option];

        if (option === '*' && result[i - 1]) {
            result[i - 1] *= options[option];
        }
